...
const PhysicalCards = (props)=>{
  ...
  const fetchData = async ()=>{
        ...
        (async ()=>{
          ...
          const {data: paymentsData,} = await supabase.from('payment_types').select('*, type_of(*)').eq('type_of', 2)
            if(paymentsData){
          ...
          const {data: paymentTypesData, error: paymentTypesError} = await supabase.from('payment_types').select('*, type_of(*)').eq('id', 2)
            ...
            zheader && await supabase.from('z_physical_cards').select('*, ptid(*), zheader: zheader_id(*), rate: rate(*)').eq('zheader_id', zheader).then(({data})=>{
              if(data) setRows(data)
            });
            supabase.from('z_phySsical_cards').on('*', async payload=>{
              const {data} = await supabase.from('z_physical_cards').select('*, ptid(*), zheader: zheader_id(*), rate: rate(*)').eq('zheader_id', zheader);
              if(data){
                setRows(data);
                       ...
                       <TableRow hover key={`total-header-${index}`}>
                         ...
                         </TableCell>
                        {
                          sessions.map((session, index)=>{
                            ...
                            rowValues.push(sum);
                            return(
                              <TableCell className={classes.tableHeaders} key={`total-session-${index}`}>
                                <b>
